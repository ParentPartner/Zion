...
def display_metrics(username: str, today: date) -> Tuple[float, float]:
    """Display key daily metrics and return (earned, goal)."""
    ...
    if not today_df.empty:
        ...
        time_diffs = today_df["timestamp"].diff().dt.total_seconds() / 60  # minutes
    ...
    with col2:
        st.metric("Orders Completed", len(today_df) if not today_df.empty else 0)
    with col3:
        if eph is not None:
            realistic_eph = min(eph, 100)
            color = "good" if realistic_eph >= 25 else "normal" if realistic_eph >= 20 else "bad"
            st.metric("Earnings Per Hour", f"${realistic_eph:.2f}", color)
        else:
    ...
    with col4:
        if not today_df.empty and "miles" in today_df.columns and today_df["miles"].sum() > 0:
            ...
            color = "good" if epm >= 2.5 else "normal" if epm >= 1.5 else "bad"
            st.metric("Earnings Per Mile", f"${epm:.2f}", color)
...
def display_ai_insights(username: str, today: date) -> None:
    """Show AI earnings predictions for today and next 3 days."""
    ...
    pred_df = pd.DataFrame({"Date": dates, "Predicted Earnings": predictions})
